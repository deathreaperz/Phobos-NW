...
#pragma region TimeToBuild

// Replaces TechnoClass::Time_To_Build (thiscall, no stack args) to fold in BuildSpeedBonus.
// Ares already hooks this same address (TechnoClass_GetBuildTime) via Syringe, so this stacks
// as a plain DEFINE_HOOK instead of a static function-jump patch, which would silently race it.
// Get_BuildTimeMult is not reimplemented: it reads HouseTypeClass::CostMult/BuildtimeMult, which
// are unused Tiberian Sun leftovers never populated from Rules(MD).ini, so it is always 1.0.
DEFINE_HOOK(0x6F47A0, TechnoClass_TimeToBuild_BuildSpeedBonus, 0x5)
{
	GET(TechnoClass*, pThis, ECX);
	GET_STACK(DWORD, returnAddr, 0x0);
	...
	auto const pType = pThis->GetTechnoType();
	auto const what = pThis->WhatAmI();
	const bool isNaval = what == UnitClass::AbsID && pType->Naval;

	double scale = pType->BuildTimeMultiplier * static_cast<double>(pType->GetBuildSpeed());

	const double power = pOwner->GetPowerPercentage();
	double powerDivisor = 1.0 - (1.0 - power) * RulesClass::Instance->LowPowerPenaltyModifier;

	if (powerDivisor <= RulesClass::Instance->MinLowPowerProductionSpeed)
		powerDivisor = RulesClass::Instance->MinLowPowerProductionSpeed;

	if (power < 1.0 && powerDivisor >= RulesClass::Instance->MaxLowPowerProductionSpeed)
		powerDivisor = RulesClass::Instance->MaxLowPowerProductionSpeed;
	...
	const double multipleFactory = RulesClass::Instance->MultipleFactory;

	if (multipleFactory > 0.0)
	{
		const int factoryCount = HouseExt::Fetch(pOwner)->GetFactoryCountWithoutNonMFB(what, isNaval);

		for (int i = factoryCount - 1; i > 0; --i)
			scale *= multipleFactory;
	}

	scale *= BuildingTypeExt::GetExternalFactorySpeedBonus(pThis, pOwner);

	if (what == BuildingClass::AbsID && static_cast<BuildingTypeClass*>(pType)->Wall)
		scale *= RulesClass::Instance->WallBuildSpeedCoefficient;

	// Simulate this function's own `retn`, since we replace it entirely instead of running stolen bytes.
	R->EAX(static_cast<int>(scale));
	R->ESP(R->ESP() + 0x4);
	return returnAddr;
}
